...
public class QuizController {
    ...
    @GetMapping("/generate")
    @Operation(summary = "Generate a mixed quiz with questions from all active categories")
    public ResponseEntity<ApiResponse<List<QuizQuestion>>> generateMixedQuiz(
            @RequestParam(defaultValue = "4") int countPerCategory) {
        return ResponseEntity
                .ok(ApiResponse.ok("Mixed quiz generated", quizService.generateMixedQuiz(countPerCategory)));
    }
    ...
    @PostMapping("/submit")
    @SecurityRequirement(name = "BearerAuth")
    @Operation(summary = "Submit quiz answers and receive score", description = "Send a JSON map of { questionId: selectedOption (0-3) }")
    public ResponseEntity<ApiResponse<Map<String, Integer>>> submit(
            @RequestBody Map<Long, Integer> answers,
            @AuthenticationPrincipal UserDetails userDetails) {
        String email = userDetails != null ? userDetails.getUsername() : null;
        int score = quizService.scoreQuiz(answers, email);
        return ResponseEntity.ok(ApiResponse.ok(
                "Quiz scored and saved",
